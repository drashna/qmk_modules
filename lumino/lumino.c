...
enum {
    ...
    HIGH_BRIGHTNESS = (uint8_t)((((LUMINO_HIGH_BRIGHTNESS) <= 1.0) ? (LUMINO_HIGH_BRIGHTNESS) : 1.0) *
                                    (RGB_MATRIX_MAXIMUM_BRIGHTNESS) +
                                0.5),
    LOW_BRIGHTNESS =
        (uint8_t)((((LUMINO_LOW_BRIGHTNESS) <= 1.0) ? (LUMINO_LOW_BRIGHTNESS) : 1.0) * (RGB_MATRIX_MAXIMUM_BRIGHTNESS) +
                  0.5),
    // Compute (255 / LUMINO_TRANSITION) with 6 fractional bits.
    ANIM_TIME_SCALE_Q6 = (255 * 64 + (LUMINO_TRANSITION) / 2) / (LUMINO_TRANSITION),
...
#    if (LUMINO_SOON_TIMEOUT) > 0 && ((LUMINO_SOON_TIMEOUT) < (LUMINO_LONG_TIMEOUT) || (LUMINO_LONG_TIMEOUT) <= 0)
    if (event_count <= 10) {
...
void lumino_cycle_3_state(void) {
    lumino_set_value((awake_value == 0) ? LOW_BRIGHTNESS : ((awake_value < HIGH_BRIGHTNESS) ? HIGH_BRIGHTNESS : 0));
}
...
void housekeeping_task_lumino(void) {
    ...
    if (anim_start_time) {
        ...
        if (timer_expired(now, anim_update_time)) {
            ...
            const uint8_t t = (elapsed <= (LUMINO_TRANSITION)) ? (((uint16_t)elapsed * ANIM_TIME_SCALE_Q6) >> 6) : 255;
