 ...
 */
 ...
 custom_shift_key_t* custom_shift_keys_get(uint16_t custom_shift_keys_idx);


 bool process_record_custom_shift_keys(uint16_t keycode, keyrecord_t *record) {
    if(!process_record_custom_shift_keys_kb(keycode, record)) {
        return false;
    }
    ...
    if (record->event.pressed) { // Press event.
       ...
       // Continue default handling if this is a tap-hold key being held.
            ...
            for (int i = 0; i < custom_shift_keys_count(); ++i) {
                custom_shift_key_t* key_config = custom_shift_keys_get(i);

                if (keycode == key_config->keycode) {
                    registered_keycode = key_config->shifted_keycode;
