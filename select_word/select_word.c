...
static void select_word_in_dir(int8_t dir) {
    ...
    if (selection_dir == 0) { // Initial selection.
        send_keyboard_report();
        send_string_with_delay_P((dir < 0) ? PSTR(SS_TAP(X_LEFT) SS_TAP(X_RGHT)) : PSTR(SS_TAP(X_RGHT) SS_TAP(X_LEFT)),
                                 TAP_CODE_DELAY);
    }
...
static void select_line(void) {
    ...
    if (selection_dir != 2) {
        send_keyboard_report();
        send_string_with_delay_P(IS_MAC ? PSTR(SS_LGUI(SS_TAP(X_LEFT) SS_LSFT(SS_TAP(X_RGHT))))
...
void select_word_unregister(void) {
    ...
    if (registered_hotkey == KC_DOWN) {
        ...
        clear_all_mods();
        send_keyboard_report();
        send_string_with_delay_P(IS_MAC ? PSTR(SS_LGUI(SS_LSFT(SS_TAP(X_RGHT)))) : PSTR(SS_LSFT(SS_TAP(X_END))),
                                 TAP_CODE_DELAY);
        set_mods(saved_mods);
    }
