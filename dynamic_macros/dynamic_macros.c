...
// This size needs to be macro count (8) times macro size (64) times sizeof(keyrecord_t)+3.
// There is no fast and dirty way to calculate this, since keyrecord_t is a struct that varies in size,
// based on the features enabled
_Static_assert((sizeof(dynamic_macros)) <= EECONFIG_MODULE_DYNAMIC_MACROS_DATA_SIZE,
               "EECONFIG_MODULE_DYNAMIC_MACROS_DATA_SIZE must be large enough to host all macros");
...
bool process_record_dynamic_macro(uint16_t keycode, keyrecord_t *record) {
    if (STATE_NOT_RECORDING == recording_state) {
        ...
        if (keycode == COMMUNITY_MODULE_DYNAMIC_MACRO_PROGRAM && record->event.pressed) {
        ...
        if (IS_DYN_KEYCODE(keycode) && record->event.pressed) {
            dynamic_macro_play(keycode - COMMUNITY_MODULE_DYNAMIC_MACRO_KEY0);
        ...
        if (keycode == COMMUNITY_MODULE_DYNAMIC_MACRO_PROGRAM && record->event.pressed) {
            ...
            macro_id = keycode - COMMUNITY_MODULE_DYNAMIC_MACRO_KEY0;
...
inline uint32_t dynamic_macro_eeprom_macro_addr(uint8_t macro_id) {
    return (uint32_t)(sizeof(dynamic_macro_t) * macro_id);
}

void eeconfig_init_dynamic_macro(void) {
...
void dynamic_macro_load_eeprom(uint8_t macro_id) {
    ...
    eeconfig_read_dynamic_macros_datablock(dst, dynamic_macro_eeprom_macro_addr(macro_id), sizeof(dynamic_macro_t));
...
void dynamic_macro_save_eeprom(uint8_t macro_id) {
    ...
    eeconfig_update_dynamic_macros_datablock(src, dynamic_macro_eeprom_macro_addr(macro_id), sizeof(dynamic_macro_t));
    dprintf("dynamic macro: slot %d saved to eeprom\n", macro_id);
}

void keyboard_post_init_dynamic_macros(void) {
    /* zero out macro blocks  */
    memset(&dynamic_macros, 0, DYNAMIC_MACRO_COUNT * sizeof(dynamic_macro_t));

    if (!eeconfig_is_dynamic_macros_datablock_valid()) {
        eeconfig_init_dynamic_macros_datablock();
    }

    eeconfig_init_dynamic_macro();
